...
| Encrypted / opaque reasoning state | Official docs now enumerate `reasoning.encrypted_content` in `include`; reasoning items can carry `encrypted_content` | No stable equivalent | No stable equivalent in the public Messages wire shape | Provider-native same-wire handling preserves the opaque carrier only when no body mutation or response normalization is required and the same protocol can preserve native semantics. Under maximum safe compatibility, request-side continuity may emit a portability warning and omit the carrier only when visible summary text or visible transcript/history remains, and opaque-only reasoning fails closed. Do not synthesize fake encrypted state. Response-side reasoning encrypted_content has a separate Anthropic carrier recovery path. |
...
| Opaque reasoning state is non-portable on request input | Encrypted or otherwise opaque reasoning content has no safe cross-provider request target shape. Under maximum safe compatibility, translation may keep visible summary text or visible transcript/history as ordinary context while omitting the opaque carrier with a portability warning; opaque-only reasoning fails closed. |
| Response carrier recovery is separate | Response-side reasoning encrypted_content can use a dedicated Anthropic carrier recovery path. Do not treat request-side continuity warn-and-omit rules as the whole response translation policy. |
...
1. Preserve reasoning request knobs only through native same-wire handling when no body mutation or response normalization is required and the same protocol can preserve native semantics.
2. Preserve summarized reasoning text and usage counters when possible.
3. Preserve opaque reasoning carriers only through provider-native same-wire handling under the same no-mutation/no-normalization boundary.
4. Under maximum safe compatibility, never forward `include: ["reasoning.encrypted_content"]`, reasoning item `encrypted_content`, or proxy-local opaque thinking carriers across providers; emit a portability warning and omit the opaque carrier only when visible summary text or visible transcript/history remains.
5. Fail closed for request-side opaque/provenance reasoning continuity fields and detailed block structures that cannot be represented portably. Ordinary reasoning knobs may map, warn and omit non-portable detail, or fail closed according to the target protocol. Opaque-only reasoning state always fails closed.
