...
| Encrypted / opaque reasoning state | Official docs now enumerate `reasoning.encrypted_content` in `include`; reasoning items can carry `encrypted_content` | No stable equivalent | No stable equivalent in the public Messages wire shape | Raw/native forwarding preserves the opaque carrier when the route can avoid mutation. For request-side continuity, maximum-compatible cross-provider translation may warn/drop the carrier only when visible summary text or visible transcript/history remains, and opaque-only reasoning fails closed. Do not synthesize fake encrypted state. Response-side reasoning encrypted_content has a separate Anthropic carrier recovery path. |
...
3. Preserve opaque reasoning carriers only through raw/native forwarding.
