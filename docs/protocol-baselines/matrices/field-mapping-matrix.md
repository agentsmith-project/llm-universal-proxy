...
| Prompt-cache control | `prompt_cache_key`, retention policy | `prompt_cache_key` on supported surfaces | `cache_control` | Same-wire preserve only / Fail closed | Not the same primitive |
