...
- Proxy posture: native Gemini `generateContent` and `streamGenerateContent` are not active `llmup` surfaces. Gemini models should be configured through Google's OpenAI-compatible endpoint with `format: openai-chat-completions`.
...
- On historical Gemini-native raw/native forwarding traffic, preserve unknown or opaque fields such as `partMetadata` and real Gemini `thoughtSignature` values.
...
- A real Gemini `thoughtSignature` may be preserved only on historical Gemini-native raw/native forwarding traffic.
