...
- Status: current-main status update; internal raw same-protocol forwarding is present; coarse-grained provider-native prompt-cache request-control disposition remains visible in traces/hooks as `preserved_native` / `explicit_extension_mapped` / `synthesized` / `dropped`; debug trace request entries additionally emit fine-grained `request.prompt_cache_request_control` details for existing preserve/map/synthesize/drop behavior; OpenAI-family -> Anthropic top-level `extra_body.anthropic.cache_control` mapping, Anthropic -> OpenAI-family `extra_body.openai.prompt_cache_key` / `prompt_cache_retention` mapping, and controlled OpenAI-family `prompt_cache_key` synthesis from stable static prefixes are present; same-protocol wrong-target explicit extensions fail closed; usage hooks emit optional same-protocol zero-transform/native-preserved `ProviderCacheUsage` source-field telemetry for known OpenAI Chat, OpenAI Responses, and Anthropic cache usage fields; Conversation State Bridge supports visible reasoning summary replay and first-response streaming completed visible output capture, while `stream:true` + `previous_response_id` still fails closed
- Date: 2026-05-18
...
- Current main includes debug trace request-level `request.prompt_cache_request_control` details for existing provider prompt-cache preserve/map/synthesize/drop behavior. These details use field paths, reasons, and synthesized-key fingerprints, not prompt text or full prompt-cache key values. Hook payloads keep only the coarse `llmup.provider_prompt_cache_request_control` value and the existing `provider_cache_usage` rules.
...
struct RequestProcessingObservability {
    ...
    provider_prompt_cache_request_control: PromptCacheRequestControl, // none | preserved_native | explicit_extension_mapped | synthesized | dropped
}
```
...
3. Provider-native prompt-cache request-control mapping or controlled OpenAI-family key synthesis, if the target request supports it.
4. Upstream request.
...
The route decision should be visible in debug traces and metrics through `llmup.request_body_handling`, `llmup.local_state_handling` when capture/replay is active, and `llmup.provider_prompt_cache_request_control`. Debug trace request entries may also include `request.prompt_cache_request_control` fine-grained details for provider prompt-cache request controls or controlled OpenAI-family key synthesis. Hook payloads and external `llmup` objects keep the coarse `provider_prompt_cache_request_control` projection; `provider_cache_usage` emission rules are unchanged.
...
- Provider prompt-cache request-control support: `llmup` may preserve or explicitly map provider-native request controls, and may synthesize the controlled OpenAI-family `prompt_cache_key`, to ask the selected upstream to use its own prompt-cache mechanism.
...
- On constructed OpenAI-family target requests, synthesize `prompt_cache_key` only from the controlled canonical stable static prefix and only when no explicit key or provider-owned state control is present.
...
- Auto-insert Anthropic breakpoints, unknown provider cache fields, or OpenAI-family keys outside the controlled stable-prefix synthesis path.
...
Prompt-cache trace vocabulary is computed internally from the target request shape, explicit provider-native fields, and controlled synthesis facts. It describes the delivered preserve/map/synthesize/drop behavior and does not create a prompt-cache IR:
...
- `synthesized`: synthesize OpenAI-family `prompt_cache_key` from the controlled stable static-prefix digest; trace only the reason and key fingerprint.
...
Explicit provider-native support is an internal implementation detail under the same maximum safe compatibility goal. This plan does not define general automatic prompt-cache key or breakpoint insertion beyond the controlled OpenAI-family `prompt_cache_key` synthesis described here.
...
- Constructed OpenAI-family target requests may synthesize `prompt_cache_key` from a canonical stable static-prefix digest scoped by namespace, upstream, and model. Synthesis does not set retention and is skipped when an explicit key or provider-owned state control is present.
...
- Anthropic to OpenAI translation strips `cache_control`; any synthesized OpenAI `prompt_cache_key` comes only from the stable static-prefix digest, not from Anthropic `cache_control`, TTL, prewarm, or provider state.
...
- Debug trace request metadata for existing provider prompt-cache preserve/map/synthesize/drop behavior is delivered as `request.prompt_cache_request_control`. Hook payloads and external `llmup` observability remain coarse by design.
...
- Anthropic-shaped requests routed to OpenAI can use the explicit target-provider `extra_body.openai.prompt_cache_key` extension; otherwise the controlled stable-prefix synthesis path may add an OpenAI-family key when eligible.
- Broader mapping or shared-IR work is a non-current separate scope review item and must not add general automatic controls, `llmup` cache behavior, config, or mode.
...
Provider-native prompt-cache request-control support currently uses explicit support plus one controlled synthesis case:

- `explicit support`: the proxy only forwards or maps documented explicit provider-native extension fields.
- `controlled synthesis`: constructed OpenAI-family target requests may receive a synthesized `prompt_cache_key` based on the canonical stable static prefix when no explicit key or provider-owned state control is present.

The proxy may help express a provider-native cache request only through explicit target-provider fields or the controlled OpenAI-family synthesis path. This plan does not add Anthropic breakpoint synthesis, unknown provider cache fields, cache-aware route selection, or `llmup` cache behavior.
...
- Constructed requests to OpenAI-family targets: synthesize `prompt_cache_key` only from canonical stable static prefix, scoped to namespace/upstream/model, when no explicit key or provider-owned state control is present.
...
- Do not derive OpenAI `prompt_cache_key` from Anthropic `cache_control`, `cache_control.ttl`, `max_tokens: 0`, dynamic message text, metadata, conversation IDs, request IDs, provider credentials, `previous_response_id`, `conversation`, `resp_llmup_*`, or similar provider-owned state.
...
- Additional translated cache-control mapping is not a current deliverable. Any expansion must start with a separate scope review and must not add automatic markers beyond the controlled OpenAI-family `prompt_cache_key` synthesis path or `llmup` cache behavior.
...
- OpenAI Chat / Responses: preserve explicit `prompt_cache_key` and `prompt_cache_retention` on OpenAI-family targets. If constructing an OpenAI-family target request without an explicit key or provider-owned state control, synthesize only the controlled stable-prefix `prompt_cache_key`; do not synthesize or change retention spelling.
...
For any constructed OpenAI-family target request in this matrix, controlled
stable-prefix synthesis may add `prompt_cache_key` when no explicit key or
provider-owned state control is present.
...
- Run explicit request-control mapping and controlled OpenAI-family key synthesis after any conversation-state expansion and after translation has produced the target-provider request shape, because provider cache matching uses the exact prompt prefix, order, and parameters the target provider sees.
...
- Debug traces emit `request.prompt_cache_request_control` details for delivered preserve/map/synthesize/drop behavior: target provider, disposition, fields/sources where applicable, TTL/retention source where applicable, omit reason, synthesis reason, and synthesized-key fingerprint. Hooks and external `llmup` observability stay coarse and must not expose the full synthesized key.
...
- OpenAI: prompt caching is automatic above the provider threshold. Preserve explicit `prompt_cache_key`; controlled synthesis may add a stable-prefix `prompt_cache_key` on constructed OpenAI-family requests. Let `cached_tokens` prove effectiveness.
...
- Controlled synthesis may add `prompt_cache_key` on constructed OpenAI-family requests from a canonical stable static prefix; it must not set `prompt_cache_retention`.
...
- Delivered slice: controlled OpenAI-family `prompt_cache_key` synthesis from stable static-prefix digest, with synthesized-key redaction and trace fingerprinting, and no `llmup` cache.
- Delivered slice: prompt-cache trace/docs guardrails are in place. Debug trace request objects emit fine-grained `request.prompt_cache_request_control` details for existing provider prompt-cache preserve/map/synthesize/drop behavior; hook payloads and external `llmup` observability remain coarse; `provider_cache_usage` emission rules are unchanged; the field mapping matrix documents explicit mapping, synthesis, warn/omit, fail-closed, and not-`llmup`-cache guardrails.
...
- Pending/deferred: no prompt-cache request-control expansion is in current deliverables beyond the controlled OpenAI-family synthesis already described. Anything beyond the delivered explicit top-level extensions and synthesis path requires separate scope review. Do not add a policy/config surface for cache-aware routing or additional automatic provider cache controls.
...
- Keep provider-native prompt-cache request-control facts explicit for delivered preserve/map/synthesize/drop behavior; broader shared-IR work is not a current deliverable and requires non-current separate scope review.
...
- Delivered: controlled OpenAI-family `prompt_cache_key` synthesis from stable static-prefix digest when no explicit key or provider-owned state control is present.
...
- Do not add OpenAI `prompt_cache_key` unless the request supplied an explicit supported OpenAI extension or the controlled stable-prefix synthesis path applies.
...
- Delivered: debug trace request fields show the trace value, target provider, mapped/target fields, explicit source, optional TTL/retention source, synthesis reason, synthesized-key fingerprint, and omit reason for existing provider prompt-cache request-control behavior.
...
- Controlled synthesis emits a redacted/fingerprinted OpenAI-family `prompt_cache_key` from a stable static-prefix digest, shares the key across dynamic tail changes, and changes the key when the static prefix or tool schema changes.
- No translated path derives `prompt_cache_key` from raw dynamic prompt text, Anthropic TTL, `max_tokens: 0`, metadata, conversation IDs, request IDs, provider credentials, provider-owned state, or `resp_llmup_*`.
...
| Provider prompt-cache support | Native cache request fields and usage fields preserved; explicit translated extensions map only to their target provider; controlled OpenAI-family synthesis adds only a stable-prefix `prompt_cache_key`; optional same-protocol zero-transform/native-preserved observation does not mutate output, drive routing, or cache anything |
| Provider prompt-cache coverage | Delivered preserve/map/synthesize/drop behavior is covered across the Explicit Provider-Native Coverage Checklist; broader mapping or shared IR expansion is not current deliverable work |
...
2. Delivered: prompt-cache trace/docs guardrails. Debug trace request objects emit `request.prompt_cache_request_control` for delivered preserve/map/synthesize/drop behavior; hook payloads and external `llmup` observability remain coarse.
...
- Automatic provider cache key, marker, or breakpoint insertion beyond controlled OpenAI-family stable-prefix `prompt_cache_key` synthesis. Only provider-native controls explicitly supplied by the request may be preserved or mapped.
