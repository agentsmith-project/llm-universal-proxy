...
- 注入 `ANTHROPIC_AUTH_TOKEN=<proxy_key>`、`ANTHROPIC_BASE_URL=http://127.0.0.1:{port}/anthropic`、`CLAUDE_CONFIG_DIR=<managed dir>`；默认不注入 `CLAUDE_CODE_SUBPROCESS_ENV_SCRUB=1`，避免影响 Claude Code 原生 permission / sandbox 行为。
