...
- Cross-provider request translation fails closed for high-risk provider-state, safety, and reasoning fields that the proxy cannot faithfully replay: OpenAI Responses provider `previous_response_id`, `conversation` / `context_management`, native compact resources, and opaque-only reasoning or compaction state; Anthropic top-level `thinking` / `context_management`; and Anthropic signed, omitted, or redacted thinking blocks. Native provider fields may be preserved only when the same-wire-protocol route avoids mutation. The optional `conversation_state_bridge.mode=memory` adapter is an in-process transcript expansion adapter for non-streaming OpenAI Responses continuations that require cross-protocol request construction. It may save and replay llmup-owned `resp_llmup_*` transcript state for text messages, assistant text output, regular `function_call` / `function_call_output`, and portable `custom_tool_call` / `custom_tool_call_output`, while `store:false`, process restart, unknown/expired IDs, external provider IDs, and provider-owned opaque state still fail closed for replay.
