...
Status cells in the quick matrix answer whether the capability is officially documented on that provider surface. They are provider surface facts, not llmup product grades. Portability boundaries and warn-and-omit guidance live in the linked detailed docs.
...
hard portability boundaries. Same-wire-protocol requests that do not need
cross-protocol construction may keep provider-native bytes and fields unchanged
internally only after routing and boundary checks show that no body mutation or
response normalization is required. That is not a separate product behavior.
Cross-provider documented translation/fail-closed
behavior is the portability contract: the proxy documents supported mappings and
rejects high-risk unsupported fields before contacting upstream. Compatible
providers can satisfy live GA evidence by exposing the OpenAI-compatible chat-completions route `/openai/v1/chat/completions` and the Anthropic-compatible messages route `/anthropic/v1/messages`. MiniMax is only one example compatible
provider, not an OpenAI Responses certified clone and not a GA-required provider.

Compatibility is one product behavior: maximum safe compatibility. Same-format routes may keep native bytes and fields unchanged when the proxy can avoid body mutation and response normalization, and all request construction that needs shims, model body rewrites, response normalization, or protocol conversion should use maximum safe compatibility.

The proxy should treat function calling and explicit transcript replay as the common denominator. Hosted tools, provider-managed state, compaction, and cache-control semantics are increasingly vendor-specific and should be preserved only when same-wire-protocol internal handling can keep native semantics unchanged, or documented as non-portable detail that is omitted with a portability warning. Opaque provider state is preserved only when the route stays on the owning provider protocol and avoids mutation.
...
- Cross-provider request translation fails closed for high-risk provider-state, safety, and reasoning fields that the proxy cannot faithfully replay: OpenAI Responses provider `previous_response_id`, `conversation` / `context_management`, native compact resources, and opaque-only reasoning or compaction state; Anthropic top-level `thinking` / `context_management`; and Anthropic signed, omitted, or redacted thinking blocks. Native provider fields may be preserved only when the same-wire-protocol route avoids mutation. Built-in local transcript replay may save and replay llmup-owned `resp_llmup_*` transcript state for OpenAI Responses continuations that require cross-protocol request construction. It covers text messages, assistant text output, regular `function_call` / `function_call_output`, portable `custom_tool_call` / `custom_tool_call_output`, and first-response streaming completed text capture; `store:false`, process restart, unknown/expired IDs, namespace/owner mismatch, route/config drift, external provider IDs, provider-owned opaque state, and streaming continuation with `previous_response_id` still fail closed.
- Provider prompt-cache support is provider-native request-control support plus usage telemetry, not a `llmup` cache and not cross-provider state reconstruction: translated routes may map only explicit documented provider-native request fields such as OpenAI `prompt_cache_key` or Anthropic top-level `cache_control`, and must not fabricate unrelated provider resources or treat provider cache controls as semantically identical.
- `context_management`, compact resources, and provider-native state-control surfaces remain same-wire-protocol only and fail closed for cross-provider reconstruction. Request-side compaction input items do not forward `encrypted_content` or other opaque state across providers. A compaction item may emit a portability warning and omit opaque carrier fields only when that item has explicit visible summary text, or when the request contains non-compaction visible portable transcript/history. Opaque-only compaction always fails closed, and one summarized compaction item does not permit another opaque-only compaction item to be silently omitted. Native Responses same-wire handling should preserve the native item unchanged when no body mutation or response normalization is required.
- OpenAI Responses lifecycle and state resource endpoints, including response input items, input token counting, conversations, and conversation items, require same-wire-protocol OpenAI Responses upstream handling when the route can avoid mutation. They require one available native OpenAI Responses upstream in the selected namespace and fail closed for no upstream, multiple candidate upstreams, unavailable upstreams, or cross-provider state reconstruction.
...
- Replayable tool history requires a complete and trusted structured call. Non-replayable or truncated tool calls should fall back to text/context preservation with a portability warning rather than masquerade as valid structured replay across providers.
