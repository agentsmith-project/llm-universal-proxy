...
hard portability boundaries. Eligible same-wire-protocol requests may use
internal byte-preserving forwarding only after routing and boundary checks show
that no body mutation or response normalization is required. That is not a
separate product behavior. Cross-provider documented translation/fail-closed
