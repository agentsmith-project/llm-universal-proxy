...
hard portability boundaries. Raw same-protocol forwarding is the intended
pre-GA byte-preserving optimization for provider-native fields and lifecycle
resources when no body mutation or response normalization is required. Until
that zero-transformation forwarding work lands, same-protocol routes may still
pass through compatibility machinery. Cross-provider documented
translation/fail-closed behavior is the portability contract: the proxy
documents supported mappings and rejects high-risk unsupported fields before
contacting upstream. Compatible providers can satisfy live GA evidence by
...
only one example compatible provider, not an OpenAI Responses certified clone and
not a GA-required provider.

Compatibility is one product behavior: maximum safe compatibility. Same-format routes should use raw forwarding when the proxy can avoid body mutation and response normalization, and all request construction that needs shims, model body rewrites, response normalization, or protocol conversion should use maximum safe compatibility.

The proxy should treat function calling and explicit transcript replay as the common denominator. Hosted tools, provider-managed state, compaction, and cache-control semantics are increasingly vendor-specific and should be preserved only by raw/native byte-preserving forwarding or documented as intentional degradations. Raw/native forwarding preserves opaque provider state only when the route remains native and avoids mutation.
